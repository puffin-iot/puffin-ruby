# I'm being rude and have copied this completely from Stripe
# since I'd really like to figure out how it all works
module Puffin
  class PuffinObject
    include Enumerable
    ...
    # Determines the equality of two Puffin objects. Puffin objects are
    # considered to be equal if they have the same set of values and each one
    # of those values is the same.
    def ==(other)
      other.is_a?(PuffinObject) && @values == other.instance_variable_get(:@values)
    end
    ...
    #   the object.
    # * +opts+ - Options for +PuffinObject+ like an API key that will be reused
    #   on subsequent API calls.
    #
    # ==== Options
    #
    # * +:dirty+ - Whether values should be initiated as "dirty" (unsaved) and
    #   which applies only to new PuffinObjects being initiated under this
    #   PuffinObject. Defaults to true.
    def update_attributes(values, opts = {}, method_options = {})
      ...
      values.each do |k, v|
        @values[k] = Util.convert_to_puffin_object(v, opts)
    ...
    # Sets all keys within the PuffinObject as unsaved so that they will be
    # included with an update when #serialize_params is called. This method is
    # also recursive, so any PuffinObjects contained as values or which are
    ...
    def serialize_params(options = {})
      ...
      @values.each do |k, v|
        ...
        #   3. Its value is a PuffinObject. A PuffinObject may contain modified
        #      values within in that its parent PuffinObject doesn't know about.
        #
        unsaved = @unsaved_values.include?(k)
        if options[:force] || unsaved || v.is_a?(PuffinObject)
    ...
    def method_missing(name, *args)
      ...
      begin
        ...
        if @transient_values.include?(name)
          raise NoMethodError.new(e.message + ".  HINT: The '#{name}' attribute was set in the past, however.  It was then wiped when refreshing the object with the result returned by Puffin's API, probably as a result of a save().  The attributes currently available on this object are: #{@values.keys.join(', ')}")
        else
          raise
        end
      end
    ...
    # * +:opts:+ Options for PuffinObject like an API key.
    ...
    def serialize_params_value(value, original, unsaved, force)
      ...
      # making sure any time one is set, we convert it to a PuffinObject. This
      ...
      # existing array being held by a PuffinObject. This could happen for
      # example by appending a new hash onto `additional_owners` for an
      # account.
      when value.is_a?(Hash)
        Util.convert_to_puffin_object(value, @opts).serialize_params

      when value.is_a?(PuffinObject)
    ...
    def empty_values(obj)
      values = case obj
      when Hash         then obj
      when PuffinObject then obj.instance_variable_get(:@values)
      else
